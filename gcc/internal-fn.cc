...
expand_arith_set_overflow (tree lhs, rtx target)
{
  if (TYPE_PRECISION (TREE_TYPE (TREE_TYPE (lhs))) == 1
    ...
    write_complex_part (target, constm1_rtx, IMAG_P);
  else
    write_complex_part (target, const1_rtx, IMAG_P);
}
				    ...
				    scalar_int_mode mode, rtx res)
{
  ...
  write_complex_part (target, lres, REAL_P);
}
  ...
  if (lhs)
    {
      target = expand_expr (lhs, NULL_RTX, VOIDmode, EXPAND_WRITE);
      if (!is_ubsan)
	write_complex_part (target, const0_rtx, IMAG_P);
    }
