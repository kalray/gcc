...
k1_addr_space_convert (rtx op, tree from_type, tree to_type ATTRIBUTE_UNUSED)
{
	...
	warning (0, TYPE_ADDR_SPACE (TREE_TYPE (from_type)) == K1_ADDR_SPACE_UNCACHED
...
hwloop_optimize (hwloop_info loop)
{
  ...
  emit_label_after (loop->end_label,
		    loop->loop_end);


  insn = emit_insn (gen_loop_start (loop->iter_reg,
				    loop->iter_reg,
				    loop->end_label));
...
#define TARGET_ADDR_SPACE_LEGITIMATE_ADDRESS_P k1_addr_space_legitimate_address_p
