...
k1_expand_builtin_get (rtx target, tree args)
{
    ...
    if (regno > K1C_SFR_LAST_REGNO){
      error ("__builtin_k1_get called with illegal SFR register index %d", INTVAL(arg1));
    }
...
k1_expand_builtin_set (rtx target ATTRIBUTE_UNUSED, tree args, bool ps)
{
    ...
    if (regno > K1C_SFR_LAST_REGNO){
      error ("__builtin_k1_set called with illegal SFR register index %d", INTVAL(arg1));
    }
