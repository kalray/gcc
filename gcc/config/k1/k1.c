...
static void
k1_verify_modifier_rounding (rtx arg,  const char *mnemonic, const char *position)
{
    if (GET_CODE (arg) == CONST_INT && GET_MODE (arg) == VOIDmode) {
        ...
        if (tmp == (stmp<<shift)>>shift || tmp == (utmp<<shift)>>shift) {
            ...
            case 0: strcat(k1_modifiers, ".rn"); break;
            case 1: strcat(k1_modifiers, ".ru"); break;
            case 2: strcat(k1_modifiers, ".rd"); break;
            case 3: strcat(k1_modifiers, ".rz"); break;
            case 4: strcat(k1_modifiers, ".rna"); break;
            case 5: strcat(k1_modifiers, ".rnz"); break;
            case 6: strcat(k1_modifiers, ".ro"); break;
            default: break;
            }
            return;
        }
    }
    error ("__builtin_k1_%s expects a 3-bit field immediate %s argument.", mnemonic, position);
}
...
k1_verify_modifier_silent (rtx arg, const char *mnemonic, const char *position)
{
    ...
    error ("__builtin_k1_%s expects a boolean immediate %s argument.", mnemonic, position);
}
...
k1_modifiers_lookup(void)
{
    ...
    for (i = 0; i < sizeof(table)/sizeof(*table); i++) {
      if (!strcmp(k1_modifiers, table[i])) return table[i];
    }
