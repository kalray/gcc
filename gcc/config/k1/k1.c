...
enum k1_arch k1_architecture (void)
{
    return K1B;
}
...
{
    return count == 0
        ? get_hard_reg_initial_val (Pmode, K1B_RETURN_POINTER_REGNO)
        : NULL_RTX;
}
            ...
            for (i = 0; i < nexps; i++)
                XVECEXP (ret, 0, i) =
                    gen_rtx_EXPR_LIST (VOIDmode,
                                       gen_rtx_REG(SImode, K1B_ARGUMENT_POINTER_REGNO+i),
...
k1_expand_stack_check_allocate_stack (rtx target, rtx adjust)
{
	...
	emit_move_insn (stack_end_val,
			gen_rtx_REG (Pmode, K1B_SR2_REGNO));
    }
...
k1_emit_single_spill (int regno, enum spill_action action, int *offset, int *stack_size)
{
    ...
    enum machine_mode spill_mode = (regno == K1B_RETURN_POINTER_REGNO) ? Pmode : SImode;
	...
	if (regno == K1B_RETURN_POINTER_REGNO && action == SPILL_RESTORE) {
	    insn = emit_move_insn (gen_rtx_REG (spill_mode, K1B_RETURN_POINTER_REGNO), reg);
	}
    }
...
k1_emit_pair_spill (int regno, enum spill_action action, int *offset, int *stack_size)
{
	...
	*stack_size += 4;
	...
	if (regno == K1B_RETURN_POINTER_REGNO && action == SPILL_RESTORE) {
	    insn = emit_move_insn (gen_rtx_REG (DImode, K1B_RETURN_POINTER_REGNO), reg);
	}
    }
...
k1_spill (enum spill_action action)
{
    ...
    for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++) {
      if ((!no_save
	   || regno == K1B_RETURN_POINTER_REGNO)
	  ...
	  if ((regno % 2 == 0
		...
		if (free_pair < 0
		    && !fixed_regs [regno]
		    && !fixed_regs [regno+1]
		    && regno != K1B_STRUCT_POINTER_REGNO // do not use struct pointer
...
k1_expand_prologue (void)
{
    ...
    if (cfun->stdarg && crtl->args.info < K1B_ARG_REG_SLOTS)
        frame_size += UNITS_PER_WORD * ((K1B_ARG_REG_SLOTS - crtl->args.info + 1) & ~1);
...
k1_legitimize_gp_address (rtx x, rtx reg)
{
    ...
    if (k1_needs_gp_symbol_reloc (x, &sym))
    {
        ...
        emit_move_insn (reg, gen_rtx_PLUS (Pmode,
                                           get_hard_reg_initial_val (Pmode, K1B_GLOBAL_POINTER_REGNO),
                                           reg));
        return reg;
    }
  return x;
...
k1_expand_builtin_get (rtx target, tree args, enum machine_mode mode)
{
    ...
    if ((mode == SImode && ((REGNO_REG_CLASS_INSENSITIVE (regno) == SRF64_REGS)) && TARGET_64)){
      warning (0, "__builtin_k1_get used with a 64bits SRF $%s (%d). Maybe use __builtin_k1_getd instead", reg_names[regno], regno-K1B_SRF_FIRST_REGNO);
    }

    if (mode == DImode && ((REGNO_REG_CLASS_INSENSITIVE (regno) == SRF32_REGS))){
      error ("__builtin_k1_getd must only be used on 64bits SRF. $%s (%d) is 32bits (use __builtin_k1_get instead)", reg_names[regno], regno-K1B_SRF_FIRST_REGNO);
    }
...
k1_expand_builtin_set (rtx target ATTRIBUTE_UNUSED, tree args, bool ps, enum machine_mode mode)
{
     ...
     */
    if ((mode == SImode && ((REGNO_REG_CLASS_INSENSITIVE (regno) == SRF64_REGS)) && TARGET_64)){
      warning (0, "__builtin_k1_set used with a 64bits SRF $%s (%d). Maybe use __builtin_k1_setd instead", reg_names[regno], regno-K1B_SRF_FIRST_REGNO);
    }

    if (mode == DImode && ((REGNO_REG_CLASS_INSENSITIVE (regno) == SRF32_REGS))){
      error ("__builtin_k1_setd must only be used on 64bits SRF. $%s (%d) is 32bits (use __builtin_k1_set instead)", reg_names[regno], regno-K1B_SRF_FIRST_REGNO);
    }
                          ...
                          int ignore ATTRIBUTE_UNUSED)
{
    ...
    case K1_BUILTIN_FFMANRNWD:
        return k1_expand_builtin_ffmanwd (target, exp, fcode == K1_BUILTIN_FFMANRNWD);
    case K1_BUILTIN_FFMAWD:
    case K1_BUILTIN_FFMARNWD:
        return k1_expand_builtin_ffmawd (target, exp, fcode == K1_BUILTIN_FFMARNWD);
    case K1_BUILTIN_FFMAND:
    case K1_BUILTIN_FFMANRND:
        return k1_expand_builtin_ffmand (target, exp, fcode == K1_BUILTIN_FFMANRND);
    ...
    case K1_BUILTIN_FFMSNRND:
        return k1_expand_builtin_ffmsnd (target, exp, fcode == K1_BUILTIN_FFMSNRND);
    case K1_BUILTIN_FFMSNWD:
    case K1_BUILTIN_FFMSNRNWD:
        return k1_expand_builtin_ffmsnwd (target, exp, fcode == K1_BUILTIN_FFMSNRNWD);
    case K1_BUILTIN_FFMSWD:
    case K1_BUILTIN_FFMSRNWD:
        return k1_expand_builtin_ffmswd (target, exp, fcode == K1_BUILTIN_FFMSRNWD);
...
k1_target_trampoline_init (rtx m_tramp, tree fndecl, rtx static_chain)
{
    ...
    emit_insn (gen_iorsi3 (scratch2, scratch2, GEN_INT (
        k1_architecture () >= K1B ? (int)0x90000000 : (int)0x88000000)));
