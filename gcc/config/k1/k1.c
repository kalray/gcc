...
static bool
k1_output_addr_const_extra (FILE *, rtx);

static bool
k1_target_legitimate_address_p (enum machine_mode ATTRIBUTE_UNUSED mode,
                                rtx x, bool strict);
...
static const struct attribute_spec k1_attribute_table[] = {
    /* { name, min_len, max_len, decl_req, type_req, fn_type_req, handler, affects_type } */
...
enum addressing_mode { ADDR_INVALID, ADDR_OFFSET, ADDR_MULT, ADDR_MOD };
...
k1_has_unspec_reference_1 (rtx *x, void *data ATTRIBUTE_UNUSED)
{
    return (GET_CODE (*x) == UNSPEC
            && (XINT (*x, 1) == UNSPEC_GOT
                ...
                || XINT (*x, 1) == UNSPEC_GPREL
                || XINT (*x, 1) == UNSPEC_GPREL10
                || XINT (*x, 1) == UNSPEC_TLS
...
k1_analyze_address (rtx x, bool strict, struct k1_address *addr)
{
    ...
    if ((!current_pass || current_pass->tv_id != TV_CPROP)
	&& GET_CODE (x) == PLUS
        && k1_legitimate_address_register_p (XEXP (x, 0), strict)
        && ((CONSTANT_P (XEXP (x, 1)) && k1_legitimate_constant_p (VOIDmode, XEXP (x, 1)))
            || GET_CODE (XEXP (x, 1)) == CONST_INT)
	...
	return true;
        ...
        return true;
               ...
               && k1_legitimate_address_offset_register_p (XEXP (XEXP (x, 0), 0), strict)) {
        addr->mode = ADDR_MULT;
        addr->base_reg = XEXP (x, 1);
...
k1_target_conditional_register_usage (void)
{
  ...
  /* the following exists because there is no FDPIC ABI, simply patch the default one */
  /* if (TARGET_FDPIC){ */
  /*   /\* if we're in FDPIC, fix the r9 register *\/ */
  /*   fix_register ("r9", 1, 1); */
  /* } */
...
/* Return rtx for register where argument is passed, or zero if it is passed
   on the stack.  */
static rtx
k1_function_arg (cumulative_args_t cum_v,
...
{
  return (((mode == BLKmode ? int_size_in_bytes (type) : GET_MODE_SIZE (mode)) > 4) ? 64 : 32);
}

/* Return the number of bytes, at the beginning of an argument,
   that must be put in registers */
static int
k1_arg_partial_bytes (cumulative_args_t cum_v,
                      enum machine_mode mode,
                      tree type,
                      bool named ATTRIBUTE_UNUSED)
{
...
static void
k1_function_arg_advance (cumulative_args_t cum_v,
...
static rtx
k1_target_function_value (const_tree ret_type,
                          ...
                          bool outgoing ATTRIBUTE_UNUSED)
{
    ...
    if (mode == BLKmode
        && ((int_size_in_bytes (ret_type) * BITS_PER_UNIT)
            > (LONG_LONG_TYPE_SIZE)))
        {
            int nexps = (int_size_in_bytes (ret_type) + UNITS_PER_WORD - 1)/ UNITS_PER_WORD;
            int i;
            ...
            for (i = 0; i < nexps; i++)
                XVECEXP (ret, 0, i) =
                    gen_rtx_EXPR_LIST (VOIDmode,
                                       gen_rtx_REG(SImode, K1C_ARGUMENT_POINTER_REGNO+i),
...
static bool k1_target_asm_can_output_mi_thunk (const_tree thunk_fndecl ATTRIBUTE_UNUSED,
                                               HOST_WIDE_INT delta ATTRIBUTE_UNUSED,
                                               HOST_WIDE_INT vcall_offset ATTRIBUTE_UNUSED,
...
{
  if (GET_CODE (*x) == UNSPEC && (XINT (*x, 1) == UNSPEC_GPREL
                                  || XINT (*x, 1) == UNSPEC_GPREL10))
    return 1;
  return 0;
}
...
k1_target_vector_mode_supported_p (enum machine_mode mode)
{
  ...
  /* return V4HImode == mode || V2SImode == mode || V8SFmode == mode || V4SFmode == mode || V2SFmode == mode || V8SImode == mode; */
}
...
int k1_legitimize_reload_address (rtx* ad, int opnum, int type ATTRIBUTE_UNUSED)
{
...
static reg_class_t
k1_target_secondary_reload (bool in_p ATTRIBUTE_UNUSED,
                            rtx x ATTRIBUTE_UNUSED,
                            ...
                            secondary_reload_info *sri ATTRIBUTE_UNUSED)
{
    ...
    if (in_p && GET_CODE (x) == SYMBOL_REF
        && SYMBOL_REF_LOCAL_P (x)
        && !SYMBOL_REF_EXTERNAL_P (x)) {
        if (sri->prev_sri == NULL) {
            return GRF_REGS;
        } else {
	  sri->icode = GET_MODE(x) == SImode ? CODE_FOR_reload_in_gotoff_si : CODE_FOR_reload_in_gotoff_di ;
            return NO_REGS;
        }
...
k1_file_start ()
{
  ...
  /* Variable tracking should be run after all optimizations which change order
     of insns (like machine reorg).  It also needs a valid CFG.  This can't be done in
     k1_override_options, because flag_var_tracking is finalized after
     that.  */
...
k1_init_machine_status (void)
{
    ...
    machine = (struct machine_function *)ggc_alloc_cleared_atomic (sizeof (struct machine_function));
...
void k1_emit_stack_overflow_block (rtx* seq, rtx* last)
{
    tree handler_name;
    ...
    handler_name_rtx = gen_rtx_SYMBOL_REF (Pmode, IDENTIFIER_POINTER (handler_name));

    *seq = emit_label (cfun->machine->stack_check_block_label);
    LABEL_NUSES(cfun->machine->stack_check_block_label) = 1;
    *last = emit_call_insn (gen_call (gen_rtx_MEM (Pmode, handler_name_rtx), const0_rtx));
...
k1_get_stack_check_block (void)
{
    ...
    if (cfun->machine->stack_check_block_seq) {
        if (BLOCK_FOR_INSN (cfun->machine->stack_check_block_seq))
            return block_label (BLOCK_FOR_INSN (cfun->machine->stack_check_block_seq));
        else
...
k1_expand_stack_check_allocate_stack (rtx target, rtx adjust)
{
    ...
    rtx stack_end_sym = gen_rtx_SYMBOL_REF (Pmode, IDENTIFIER_POINTER (stack_end));
	...
	stack_end_sym = gen_rtx_UNSPEC(Pmode, gen_rtvec (1, stack_end_sym), UNSPEC_TLS);
...
k1_spill (enum spill_action action)
{
    ...
    for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++) {
	  ...
	  enum machine_mode spill_mode = (REGNO_REG_CLASS (regno) == SRF64_REGS) ? DImode : SImode;
	  ...
	  for (spill_reg = 0; spill_reg < K1C_SRF_FIRST_REGNO; spill_reg++) {
	    ...
	    bool grf_free = !save_reg[spill_reg]  // spill_reg not used in func (as scratch or for computation)
	    ...
	    bool prf_free = grf_free
	      ...
	      && !save_reg[spill_reg + 1]  // spill_reg not used in func (as scratch or for computation)
...
k1_expand_prologue (void)
{
    ...
    rtx (*gen_set_gotp) (rtx target, rtx op1, rtx op2, rtx op3) = TARGET_64 ? gen_set_gotp_di : gen_set_gotp_si;
    ...
    if (flag_pic /* && !TARGET_FDPIC */ && crtl->uses_pic_offset_table) {
       ...
       * the offset between the beginning of this function and the Global Offset Table
       */
        ...
        insn = emit_insn (gen_set_gotp(pic_offset_table_rtx, gen_rtx_REG(Pmode, PC_REGNUM),
                                       tmp1_reg, gp_disp));

        /* All we need right now is to add the program counter value and the displacement
         * to get the runtime address of the Global Offset Table
         */
        insn = emit_insn (gen_add(pic_offset_table_rtx, pic_offset_table_rtx, tmp1_reg));
    ...
    if (cfun->stdarg && crtl->args.info < K1C_ARG_REG_SLOTS)
        frame_size += UNITS_PER_WORD * ((K1C_ARG_REG_SLOTS - crtl->args.info + 1) & ~1);

    frame_size += crtl->args.pretend_args_size;

    if (frame_size != 0) {
        insn = GEN_INT (-frame_size);
        insn = emit_insn (gen_add (stack_pointer_rtx, stack_pointer_rtx,
				   insn));
        RTX_FRAME_RELATED_P (insn) = 1;
    }
    ...
    if (frame_size != 0 && flag_stack_check == FULL_BUILTIN_STACK_CHECK) {
        ...
        emit_cmp_and_jump_insns (stack_end_val, const0_rtx, LT,
                                 NULL_RTX, Pmode, 0, label);
        JUMP_LABEL(get_last_insn ()) = label;
    }
    ...
    if (frame_pointer_needed) {
        ...
        insn = GEN_INT (frame_size);
        insn = emit_insn (gen_add (frame_pointer_rtx, stack_pointer_rtx,
				   insn));
        RTX_FRAME_RELATED_P (insn) = 1;
    }
}
...
k1_expand_epilogue (void)
{
    ...
    if (frame_pointer_needed) {
        ...
        insn = emit_insn (gen_add (stack_pointer_rtx,
				   frame_pointer_rtx, GEN_INT (-frame_ptr_offset)));
        RTX_FRAME_RELATED_P (insn) = 1;
    }
...
k1_legitimize_tls_reference (rtx x)
{
    ...
    emit_move_insn (reg, gen_rtx_CONST (Pmode, gen_rtx_UNSPEC (Pmode, gen_rtvec (1, x), UNSPEC_TLS)));

    emit_insn (gen_add (reg,
			gen_rtx_REG (Pmode, K1C_LOCAL_POINTER_REGNO),
			reg));
    return reg;
}
...
k1_legitimize_gp_address (rtx x, rtx reg)
{
    ...
    if (k1_needs_gp_symbol_reloc (x, &sym))
    {
        ...
        emit_move_insn (reg, gen_rtx_CONST (Pmode,
					    gen_rtx_UNSPEC (Pmode, gen_rtvec (1, x),
							    unspec)));

        emit_move_insn (reg, gen_rtx_PLUS (Pmode,
                                           get_hard_reg_initial_val (Pmode, K1C_GLOBAL_POINTER_REGNO),
                                           reg));
        return reg;
    }
  return x;
...
k1_handle_label_or_readonly(rtx addr, rtx reg)
{
  ...
  new_rtx = gen_rtx_PLUS (Pmode, k1_pic_register_initial_val (),
                          new_rtx);
...
k1_handle_symbol(rtx addr, rtx reg)
{
  ...
  new_rtx = gen_rtx_CONST (Pmode, new_rtx);
  new_rtx = gen_rtx_PLUS (Pmode,
                          k1_pic_register_initial_val (),
                          new_rtx);
...
/* Checkout if we don't have to add UNSPEC in const and plus */
  else if (GET_CODE (addr) == CONST || GET_CODE (addr) == PLUS)
    {
      ...
      if (XEXP (addr, 0) == has_hard_reg_initial_val (Pmode, PIC_OFFSET_TABLE_REGNUM))
        return orig;
      ...
      if (GET_CODE (base) == UNSPEC
          ...
          XVECEXP (base, 0, 0) = gen_rtx_PLUS (Pmode, XVECEXP (base, 0, 0), addr);
          return base;
      }
    ...
    if (k1_has_tls_reference (operands[1]))
    ...
    else if (flag_pic /* || TARGET_FDPIC */)
      {
        if (SYMBOLIC_CONST(operands[1]))
          operands[1] = k1_target_legitimize_pic_address (operands[1], operands[0]);
      }
    else if (TARGET_GPREL) {
      operands[1] = k1_legitimize_gp_address(operands[1], operands[0]);
    }
    /* FIXME AUTO: this is needed because we can't materialize 64bits immediate yet */
      ...
      /* emit_insn(tmp, gen_anddi3(gen_rtx_CONST_INT (VOIDmode, 0xFFFFFFFFULL<<32), */
      /* 				operands[1]));. */

      emit_move_insn (tmp, gen_rtx_CONST_INT(VOIDmode, (const_val & (0xFFFFFFFFULL<<32))>>32));

      emit_insn (gen_ashldi3 (tmp, tmp, gen_rtx_CONST_INT (VOIDmode, 32)));

      emit_move_insn(operands[0], gen_rtx_CONST_INT(VOIDmode, const_val & 0xFFFFFFFF));
...
/* static void */
/* k1_generic_expand_sync_instruction (enum rtx_code code, rtx dest, rtx addr, rtx val, */
...
k1_target_init_builtins (void)
{
  ...
  const_intQI_node = build_qualified_type (intQI_type_node,
					   TYPE_QUAL_CONST);
  const_intHI_node = build_qualified_type (intHI_type_node,
					   TYPE_QUAL_CONST);
  const_intSI_node = build_qualified_type (intSI_type_node,
					   TYPE_QUAL_CONST);
  const_intDI_node = build_qualified_type (intDI_type_node,
					   TYPE_QUAL_CONST);
  const_float_node = build_qualified_type (float_type_node,
					   TYPE_QUAL_CONST);
...
#define ADD_K1_BUILTIN(UC_NAME, LC_NAME, ...)                           \
  ...
  /* FIXME AUTO: disabling vector support */
    ...
    /* ADD_K1_BUILTIN (FCMAWD,  "fcmawd",      floatDF,floatSF,floatSF,floatSF,floatSF); */
    /* ADD_K1_BUILTIN (FCMS,    "fcms",        floatSF,floatSF,floatSF,floatSF,floatSF); */
    /* ADD_K1_BUILTIN (FCMSWD,  "fcmswd",      floatDF,floatSF,floatSF,floatSF,floatSF); */
    /* ADD_K1_BUILTIN (FDMA,    "fdma",        floatSF,floatSF,floatSF,floatSF,floatSF); */
    /* ADD_K1_BUILTIN (FDMAWD,  "fdmawd",      floatDF,floatSF,floatSF,floatSF,floatSF); */
    /* ADD_K1_BUILTIN (FDMS,    "fdms",        floatSF,floatSF,floatSF,floatSF,floatSF); */
    /* ADD_K1_BUILTIN (FDMSWD,  "fdmswd",      floatDF,floatSF,floatSF,floatSF,floatSF); */
    ...
    ADD_K1_BUILTIN (FFMANWDRN,"ffmanwdrn",  floatDF,floatDF,floatSF,floatSF);
...
verify_const_int_arg (rtx arg, int bits, bool signed_p)
{
    if (GET_CODE (arg) != CONST_INT
        && (GET_CODE (arg) != CONST_DOUBLE
            || GET_MODE (arg) != VOIDmode))
        return false;

    if (signed_p) {
        signed long long tmp = CONST_INT_P(arg) ?
            INTVAL(arg) : CONST_DOUBLE_LOW(arg);
        return tmp >= ~((1LL << (bits-1))-1)
            && tmp <= ((1LL << (bits-1))-1);
    } else {
        unsigned long long tmp = (unsigned int)(CONST_INT_P(arg) ?
                                                INTVAL(arg) : CONST_DOUBLE_LOW(arg));
...
k1_expand_builtin_get (rtx target, tree args, enum machine_mode mode)
{
    ...
    if (regno > K1C_SRF_LAST_REGNO){
      error ("__builtin_k1_get%s called with illegal SRF register index : %d", mode == SImode ? "" : "d",  INTVAL(arg1));
    }

    /* 32bits mode, double access to 64bits register : may be sign of
       error except for $ps */
    if (mode == DImode &&
	(REGNO_REG_CLASS_INSENSITIVE (regno) == SRF64_REGS) &&
	!TARGET_64 &&
	(regno !=  K1C_PS_REGNO)){
      warning (0, "__builtin_k1_getd should only be used in 64bits mode. Maybe use __builtin_k1_get.");
    }

    /*
     * 32bit access to a 64bits SRF in 64bits mode
     */
    if ((mode == SImode && ((REGNO_REG_CLASS_INSENSITIVE (regno) == SRF64_REGS)) && TARGET_64)){
      warning (0, "__builtin_k1_get used with a 64bits SRF $%s (%d). Maybe use __builtin_k1_getd instead", reg_names[regno], regno-K1C_SRF_FIRST_REGNO);
    }

    if (mode == DImode && ((REGNO_REG_CLASS_INSENSITIVE (regno) == SRF32_REGS))){
      error ("__builtin_k1_getd must only be used on 64bits SRF. $%s (%d) is 32bits (use __builtin_k1_get instead)", reg_names[regno], regno-K1C_SRF_FIRST_REGNO);
    }
...
static rtx
k1_expand_builtin_set (rtx target ATTRIBUTE_UNUSED, tree args, bool ps, enum machine_mode mode)
{
    rtx arg1 = expand_normal (CALL_EXPR_ARG (args, 0));
    ...
    if (regno > K1C_SRF_LAST_REGNO){
      error ("__builtin_k1_set%s called with illegal SRF register index : %d", mode == SImode ? "" : "d",  INTVAL(arg1));
    }
	...
	!TARGET_64 &&
	(regno !=  K1C_PS_REGNO)){
      warning (0, "__builtin_k1_setd on $%s (%d) should only be used in 64bits mode. Maybe use __builtin_k1_set instead",
     ...
     */
    if ((mode == SImode && ((REGNO_REG_CLASS_INSENSITIVE (regno) == SRF64_REGS)) && TARGET_64)){
      warning (0, "__builtin_k1_set used with a 64bits SRF $%s (%d). Maybe use __builtin_k1_setd instead", reg_names[regno], regno-K1C_SRF_FIRST_REGNO);
    }

    if (mode == DImode && ((REGNO_REG_CLASS_INSENSITIVE (regno) == SRF32_REGS))){
      error ("__builtin_k1_setd must only be used on 64bits SRF. $%s (%d) is 32bits (use __builtin_k1_set instead)", reg_names[regno], regno-K1C_SRF_FIRST_REGNO);
    }
...
k1_expand_builtin_srfsize (rtx target, tree args)
{
    ...
    reg_size_val = (TARGET_64 && ((REGNO_REG_CLASS (regno) == SRF64_REGS))) ? 8 : 4 ;
...
{
  rtx (*gen_afda_cachemode) (rtx target, rtx op1, rtx op2, rtx op3) = usecache ? gen_afda : gen_afdau;
  ...
  emit_insn (gen_afda_cachemode (target,
				 mem_target,
				 addend_and_return,
...
k1_expand_builtin_float (rtx target, tree args, bool usigned)
{
    ...
    if (!verify_const_int_arg (arg1, 2, false)) {
        error ("__builtin_k1_float%s expects an unsigned 2bits immediate first argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }

    if (!verify_const_int_arg (arg3, 6, false)) {
        error ("__builtin_k1_float%s expects an unsigned 6 bits immediate third argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }
...
k1_expand_builtin_floatd (rtx target, tree args, bool usigned)
{
    ...
    if (!verify_const_int_arg (arg1, 2, false)) {
        error ("__builtin_k1_float%sd expects an unsigned 2bits immediate first argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }

    if (!verify_const_int_arg (arg3, 6, false)) {
        error ("__builtin_k1_float%sd expects an unsigned 6 bits immediate third argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }
...
k1_expand_builtin_fixed (rtx target, tree args, bool usigned)
{
    ...
    if (!verify_const_int_arg (arg1, 2, false)) {
        error ("__builtin_k1_fixed%s expects an unsigned 2bits immediate first argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }

    if (!verify_const_int_arg (arg3, 6, false)) {
        error ("__builtin_k1_fixed%s expects an unsigned 6 bits immediate third argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }
...
k1_expand_builtin_fixedd (rtx target, tree args, bool usigned)
{
    ...
    if (!verify_const_int_arg (arg1, 2, false)) {
        error ("__builtin_k1_fixed%sd expects an unsigned 2bits immediate first argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }

    if (!verify_const_int_arg (arg3, 6, false)) {
        error ("__builtin_k1_fixed%sd expects an unsigned 6 bits immediate third argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }
...
k1_expand_builtin_ffmanwd (rtx target, tree args, int rn)
{
    ...
    if (rn)
      emit_insn (gen_ffmanwdrn (target, arg1, arg2, arg3));
    else
...
k1_expand_builtin_ffmsnwd (rtx target, tree args, int rn)
{
    ...
    if (rn)
      emit_insn (gen_ffmsnwdrn (target, arg1, arg2, arg3));
    else
...
k1_expand_builtin_ffmand (rtx target, tree args, int rn)
{
    ...
    if (rn)
      emit_insn (gen_ffmandrn (target, arg2, arg3, arg1));
    else
...
k1_expand_builtin_ffmsnd (rtx target, tree args, int rn)
{
    ...
    if (rn)
      emit_insn (gen_ffmsndrn (target, arg2, arg3, arg1));
    else
...
k1_expand_builtin_waitany (rtx target, tree args)
{
    ...
    if (!verify_const_int_arg (arg2, 6, false)) {
        error ("__builtin_k1_waitany expects a 6 bits immediate second argument.");
        return NULL_RTX;
    }
...
k1_expand_builtin_wantany (rtx target, tree args)
{
    ...
    if (!verify_const_int_arg (arg2, 6, false)) {
        error ("__builtin_k1_wantany expects a 6 bits immediate second argument.");
        return NULL_RTX;
    }
                          ...
                          int ignore ATTRIBUTE_UNUSED)
{
    ...
    /*     return k1_expand_builtin_faddwp (target, exp, fcode == K1_BUILTIN_FADDRNWP); */
    ...
    /*     return k1_expand_builtin_fsbfwp (target, exp, fcode == K1_BUILTIN_FSBFRNWP); */
    ...
    /*     return k1_expand_builtin_ffmawp (target, exp, fcode == K1_BUILTIN_FFMARNWP); */
    ...
    /*     return k1_expand_builtin_ffmanwp (target, exp, fcode == K1_BUILTIN_FFMANRNWP); */
    case K1_BUILTIN_FFMANWD:
    case K1_BUILTIN_FFMANWDRN:
        return k1_expand_builtin_ffmanwd (target, exp, fcode == K1_BUILTIN_FFMANWDRN);
    case K1_BUILTIN_FFMAWD:
    case K1_BUILTIN_FFMAWDRN:
        return k1_expand_builtin_ffmawd (target, exp, fcode == K1_BUILTIN_FFMAWDRN);
    case K1_BUILTIN_FFMAND:
    case K1_BUILTIN_FFMANDRN:
        return k1_expand_builtin_ffmand (target, exp, fcode == K1_BUILTIN_FFMANDRN);
    ...
    /*     return k1_expand_builtin_ffmswp (target, exp, fcode == K1_BUILTIN_FFMSRNWP); */
    ...
    /*     return k1_expand_builtin_ffmsnwp (target, exp, fcode == K1_BUILTIN_FFMSNRNWP); */
    case K1_BUILTIN_FFMSND:
    case K1_BUILTIN_FFMSNDRN:
        return k1_expand_builtin_ffmsnd (target, exp, fcode == K1_BUILTIN_FFMSNDRN);
    case K1_BUILTIN_FFMSNWD:
    case K1_BUILTIN_FFMSNWDRN:
        return k1_expand_builtin_ffmsnwd (target, exp, fcode == K1_BUILTIN_FFMSNWDRN);
    case K1_BUILTIN_FFMSWD:
    case K1_BUILTIN_FFMSWDRN:
        return k1_expand_builtin_ffmswd (target, exp, fcode == K1_BUILTIN_FFMSWDRN);
    ...
    case K1_BUILTIN_FLOATUD:
        return k1_expand_builtin_floatd (target, exp, fcode == K1_BUILTIN_FLOATUD);
    ...
    /*     return k1_expand_builtin_fmulwp (target, exp, fcode == K1_BUILTIN_FMULRNWP); */

    case K1_BUILTIN_FMULWD:
    case K1_BUILTIN_FMULRNWD:
        return k1_expand_builtin_fmulwd (target, exp, fcode == K1_BUILTIN_FMULRNWD);
    case K1_BUILTIN_FMULNWD:
    case K1_BUILTIN_FMULNRNWD:
        return k1_expand_builtin_fmulnwd (target, exp, fcode == K1_BUILTIN_FMULNRNWD);

	/* FIXME AUTO: disabling vector support */
    /* case K1_BUILTIN_FMULNWP: */
    /* case K1_BUILTIN_FMULNRNWP: */
    /*     return k1_expand_builtin_fmulnwp (target, exp, fcode == K1_BUILTIN_FMULNRNWP); */
    ...
    case K1_BUILTIN_SET_PS:
      return k1_expand_builtin_set (target, exp, fcode == K1_BUILTIN_SET_PS, SImode);
	/* FIXME AUTO: disabling vector support */
                                   ...
                                   int stdinc)
{
	...
	/* Seamlessly change the board to csp_generic if developer, emb01, ab04, konic80, or tc */
...
static int k1_target_sched_dfa_new_cycle (FILE *dump ATTRIBUTE_UNUSED,
...
static bool
k1_replace_reg_rtx (int regno, rtx old, rtx replace, rtx ignore1, rtx ignore2, int remove_notes, int force_success)
{
    ...
    for (adef = DF_REG_DEF_CHAIN (regno);
         adef;
         adef = next) {
    ...
    for (adef = DF_REG_USE_CHAIN (regno);
         adef;
         adef = next) {
...
k1_add_reginfo (int regno, int old_regno1, int old_regno2)
{
    ...
    sched_regno_pressure_class = (enum reg_class *)
      xrealloc (sched_regno_pressure_class, max_regno * sizeof (enum reg_class));
    resize_reg_info ();
    ...
    REG_N_DEATHS (regno) = MAX (REG_N_DEATHS (old_regno1),
                                REG_N_DEATHS (old_regno2));
    REG_N_DEATHS (regno) = MAX (REG_N_DEATHS (old_regno1),
                                REG_N_DEATHS (old_regno2));
    REG_LIVE_LENGTH (regno) = MAX (REG_LIVE_LENGTH (old_regno1),
                                   REG_LIVE_LENGTH (old_regno2));
    REG_N_CALLS_CROSSED (regno) = MAX (REG_N_CALLS_CROSSED (old_regno1),
                                       REG_N_CALLS_CROSSED (old_regno2));
    ...
    REG_BASIC_BLOCK (regno) = MAX (REG_BASIC_BLOCK (old_regno1),
                                   REG_BASIC_BLOCK (old_regno2));
...
k1_patch_mem_for_double_access_1 (rtx *x, void *data ATTRIBUTE_UNUSED)
{
    if (MEM_P (*x)) {
        *x = shallow_copy_rtx(*x);
        XEXP(*x, 0) = plus_constant (Pmode, XEXP(*x, 0), GET_MODE_SIZE(GET_MODE(*x)));
        return 1;
    }
                                 ...
                                 rtx *insn_to_remove)
{
    ...
    for (x = addr1->base_reg;
         x != NULL;
         x = (x == addr1->base_reg) ? addr2->base_reg : NULL) {
        if (DF_REG_DEF_COUNT (REGNO (x)) != 1)
	  // Previously, this was not considered an error.
	  // Simple case where r1 is defined outside a loop and r3/r2 is redefined inside a loop:
	  // Mem access must not be packed.
	  //
                             ...
                             rtx *insn_to_remove2)
{
    ...
    for (x = offset1; x != NULL; x = (x == offset1) ? offset2 : NULL) {
        if (DF_REG_DEF_COUNT (REGNO (x)) != 1)
	  // Previously, this was not considered an error.
	  // Simple case where r1 is defined outside a loop and r3 is redefined inside a loop:
	  // Mem access must not be packed.
	  //
	  // This failure should be made less restrictive as it
	  // currently prevents most mem packing inside a loop
    ...
    if (diff && CONST_INT_P (diff))
        diff = simplify_rtx (gen_rtx_MULT (SImode,
                                           diff,
                                           GEN_INT (addr1->mult)));
    return diff;
}
                               ...
                               rtx *insn_to_remove)
{
    ...
    if (DF_REF_CLASS (ref) == DF_REF_ARTIFICIAL)
        return NULL_RTX;
    def_rtx = DF_REF_INSN (ref);
    if (DF_REG_USE_COUNT (REGNO (reg_to_replace)) == 1
        && insn_to_remove)
        *insn_to_remove = def_rtx;
    def_rtx = single_set (def_rtx);
    if (!def_rtx)
    ...
    return  simplify_rtx (gen_rtx_MINUS (Pmode,
                                         XEXP (mem2, 0),
                                         XEXP (mem1, 0)));
}
...
/* Tests if mem X loaded/stored to regno REGNO_X and mem Y
   ...
   addresses.  */
static rtx
k1_contiguous_mem_access_p (enum machine_mode mode ATTRIBUTE_UNUSED,
                            rtx x, int regno_x,
                            rtx y, int regno_y,
                            rtx *insn_to_remove,
                            rtx *insn_to_remove2)
{
    ...
    if ((regno_x < FIRST_PSEUDO_REGISTER
         && regno_y >= FIRST_PSEUDO_REGISTER)
        || (regno_x >= FIRST_PSEUDO_REGISTER
            && regno_y < FIRST_PSEUDO_REGISTER))
        return NULL_RTX;

    if (regno_y < FIRST_PSEUDO_REGISTER
        && abs(regno_y - regno_x) != 1)
        return NULL_RTX;
    ...
    diff = simplify_rtx (gen_rtx_MINUS (Pmode,
                                        XEXP (y, 0), XEXP (x, 0)));
    ...
    if (diff == NULL_RTX || !CONST_INT_P (diff)) {
        ...
        if (addr1.mode == ADDR_MULT && addr2.mode == ADDR_OFFSET) {
            y = copy_rtx (y);
            return k1_diff_addr_offset_addr_mult (mode, x, y,
                                                  y, addr2.base_reg,
                                                  insn_to_remove);
...
enum pass { PACK_LOADS, PACK_STORES };
 ...
 */
static int
k1_interesting_mem_access (int pass, enum machine_mode mode,
                           rtx x, rtx *mem, enum rtx_code *extension, int *regno)
{
    if (pass == PACK_LOADS) {
        if (!x
            || !k1_is_mem (SET_SRC (x), mem, extension)
            || MEM_VOLATILE_P (*mem)
            || !REG_P (SET_DEST (x)))
            return 0;
        *regno = REGNO (SET_DEST (x));
        ...
        if (!x
            || !k1_is_mem (SET_DEST (x), mem, extension)
            || MEM_VOLATILE_P (*mem)
            || !REG_P (SET_SRC (x)))
            return 0;
...
static int k1_lsu_load_p (rtx insn)
{
...
static int k1_lsu_store_p (rtx insn)
{
    switch (get_attr_type (insn)) {
    case TYPE_LSU_STORE:
...
k1_scan_insn_registers_1 (rtx *x, void *data)
{
    ...
    if (REG_P (*x)) {
        if (REGNO (*x) > FIRST_PSEUDO_REGISTER) {
            warning (0, "Pseudo register found while scanning final asm. Please report.");
        }
...
void
k1_final_prescan_insn (rtx insn,
                       ...
                       int nops ATTRIBUTE_UNUSED)
{
    ...
    if (!can_issue
        ...
        || bundle_length > 8*4) {
        if (!emited_colon)
            fprintf (asm_out_file, "\t;; /* Can't issue next in the same bundle */\n");
        state_reset (state);
...
static int k1_target_sched_first_cycle_multipass_dfa_lookahead (void)
{
    return 5;
}
                                 ...
                                 bool speed ATTRIBUTE_UNUSED)
{
    ...
    if (GET_CODE (x) == SET) {
	    ...
	    *total = k1_target_register_move_cost (GET_MODE (SET_DEST (x)), NO_REGS, NO_REGS);
	    return true;
	}
        x = SET_SRC (x);
    }
...
k1_target_sched_set_sched_flags (struct spec_info_def *spec ATTRIBUTE_UNUSED)
{
    ...
    if (!reload_completed
        && common_sched_info->sched_pass_id != SCHED_SMS_PASS) {
        df_chain_add_problem (DF_DU_CHAIN | DF_UD_CHAIN);
    }
}

static void k1_target_sched_init_global (FILE *file ATTRIBUTE_UNUSED,
                                         int verbose ATTRIBUTE_UNUSED,
...
static void k1_target_sched_finish_global (FILE *file ATTRIBUTE_UNUSED,
                                           int verbose ATTRIBUTE_UNUSED)
{
    if (packed_mems) {
		...
		emit_insn_before (gen_rtx_CLOBBER (DImode, action->reg), action->insn);
                ...
                /* Replace the register it defines everywhere
                   ...
                   acesses.)  */
                if ((set = single_set (insn_to_remove))
                    && REG_P (SET_DEST (set))
                    && k1_replace_reg_rtx (REGNO (SET_DEST (set)),
                                           SET_DEST (set),
                                           SET_SRC (set),
                                           insn_to_remove, NULL, true, false)) {
...
static bool
k1_function_ok_for_sibcall (tree decl,
                            tree exp ATTRIBUTE_UNUSED)
{
    /* Sometimes GCC allocates a call_used_register to hold the sibcall destination...
       This of course  won't work because we can't restore that register to the right value
       and do the call (In fact we could by bundling the igoto and the restore together).
...
clobber_reg (rtx *call_fusage, rtx reg)
{
  ...
  *call_fusage
    = gen_rtx_EXPR_LIST (VOIDmode, gen_rtx_CLOBBER (VOIDmode, reg), *call_fusage);
}
...
k1_expand_call (rtx fnaddr, rtx arg, rtx retval, bool sibcall)
{
    ...
    /*         emit_move_insn (pic_offset_table_rtx, gen_rtx_MEM (SImode, plus_constant (Pmode, funcdesc_addr, 4))); */
    /*     } else { */
                          ...
                          unsigned int counter_regno)
{
    ...
    if (BLOCK_FOR_INSN (entry)->next_bb != loop->header) {
        ...
        remove_insn (note);
        add_insn_before (note,
                         BB_HEAD (loop->header), NULL);
        ...
        e->dest->aux = e->dest->next_bb == EXIT_BLOCK_PTR_FOR_FN (cfun) ? NULL : e->dest->next_bb;
        e->src->aux = e->src->next_bb == EXIT_BLOCK_PTR_FOR_FN (cfun) ? NULL : e->src->next_bb;
        loop->header = e->dest;
     ...
     *                                    (const_int -1 [0xffffffffffffffff]))) io_main.cpp:8 7 {*addsi3_small}
     *                     (nil))
     *
     * (insn:TI 271 254 251 4 (set (mem/c:SI (plus:SI (reg/f:SI 10 r10)
     *                                       (const_int 12 [0xc])) [6 %sfp+12 S4 A32])
     *                             (reg:SI 9 r9 [184])) io_main.cpp:8 228 {*real_movsi}
     *                     (nil))
     *
     * With the memory being read after the loop.
     *
     * The internal issue is T2920 "C/C++ code may crash when compiling with -03 due to bad hw loop generation"
     *
    ...
    FOR_BB_INSNS_SAFE (loop->header, insn, next)
        if (insn != entry && INSN_P (insn)) {
            ...
            if (condjump_p (insn)) {
              ...
              rtx label = gen_rtx_LABEL_REF (Pmode,
                                             block_label (loop->header));
    ...
    if (number_of_counter_usage == 1){
	  ...
	  if (reg_mentioned_p (SET_DEST (PATTERN(entry)), PATTERN (insn))) {
	    ...
	    const bool is_counter_decr = (GET_CODE (PATTERN(insn)) == SET) 
	      && REG_P(SET_DEST(PATTERN(insn))) 
	      && (REGNO(SET_DEST(PATTERN(insn))) == counter_regno) /* target is counter */
	      && (GET_CODE(SET_SRC(PATTERN(insn))) == PLUS)
	      && REG_P(XEXP (SET_SRC(PATTERN(insn)), 0))
	      && REGNO(XEXP (SET_SRC(PATTERN(insn)), 0)) == counter_regno
	      && CONST_INT_P(XEXP (SET_SRC(PATTERN(insn)), 1));

	    /* be safe, do not kill insn if it does not look like the decrement */
	    if (is_counter_decr)
	      delete_insn (insn);
	  }
	}
    ...
    */
    ...
    FOR_BB_INSNS (loop->header, insn) {
        ...
        if (any_condjump_p (insn)) {
            ...
            x = PATTERN (insn);
            if (!REG_P (SET_DEST (x))
                || REGNO (SET_DEST (x)) != counter_regno)
                break;
    ...
    if (decrement && branch) {
        ...
        if (uses && uses->next && uses->next->next) {
            basic_block b, prev, succ;
            rtx make = gen_movsi (gen_rtx_REG (SImode, counter_regno),
                                  const0_rtx);
        ...
        while (counter_defs) {
            ...
            */
            ...
            if (counter_defs->ref->base.cl != DF_REF_ARTIFICIAL
                && DF_REF_CHAIN (df_find_def (counter_defs->ref->base.insn_info->insn,
                                              regno_reg_rtx[counter_regno]))->next != NULL) {
                          ...
                          struct df_link *counter_defs)
{
    ...
    if (BLOCK_FOR_INSN (entry)->next_bb != loop->header) {
        ...
        /* In the simple case of a 2-block loop, we can just move the
           loopdo instruction in the loop bb.  */
        /* The original 'entry' will be deleted in the cleanup pass.  */
        entry = emit_insn_before (copy_rtx (PATTERN (entry)),
                                  BB_HEAD (loop->header));
        BB_HEAD (loop->header) = entry;
        set_block_for_insn (entry, loop->header);
    ...
    if (loop->latch->prev_bb != loop->header) {
        ...
        loop->latch->aux = loop->latch->next_bb == EXIT_BLOCK_PTR_FOR_FN (cfun) ? NULL : loop->latch->next_bb;
    ...
    /* latch has no fallthrough, but we'll add one with
       ...
       the CFG also. */
    make_edge (bb, bb->aux ? (basic_block)bb->aux : EXIT_BLOCK_PTR_FOR_FN (cfun), 0);
    ...
    if (bb->aux && find_edge (loop->header, (basic_block)bb->aux)) {
        k1_optimize_two_block_loop (loop, entry, bb,
                                    counter_regno, counter_defs);
    }
}
...
k1_reorg_hwloops (void)
{
	...
	if (where != entry) {
        ...
        if (loop->num_nodes == 1) {
            ...
            k1_handle_two_block_loop (loop, entry, counter_regno,
                                      counter_defs);
        }
    }
...
k1_option_override (void)
{
  ...
  k1_small_data_threshold = (global_options_set.x_g_switch_value
                               ? g_switch_value
                               : K1C_DEFAULT_GVALUE);
...
static bool
k1_mode_dependent_address_p (const_rtx addr ATTRIBUTE_UNUSED, addr_space_t space ATTRIBUTE_UNUSED)
{
...
bool
k1_float_fits_bits(const REAL_VALUE_TYPE *r, unsigned bitsz, enum machine_mode mode){
  long l[2];
  gcc_assert(mode == DFmode || mode == SFmode);

  if (mode == SFmode){
    REAL_VALUE_TO_TARGET_SINGLE(*r, l[0]);
    return (l[0] == 0) || (8*sizeof(l[0])- __builtin_clzl(l[0])) <= bitsz;
  } else { /*  (mode == DFmode) */
    REAL_VALUE_TO_TARGET_DOUBLE(*r, l);
    if (bitsz > 32) {
      if (l[1] == 0){
...
void k1_profile_hook (void)
{
  ...
  emit_library_call (fun, LCT_NORMAL, VOIDmode, 2, mem_p_saved_ra, Pmode, fp, Pmode);
}
...
#define TARGET_VECTORIZE_SUPPORT_VECTOR_MISALIGNMENT k1_target_support_vector_misalignment
...
#define TARGET_SCHED_FIRST_CYCLE_MULTIPASS_DFA_LOOKAHEAD k1_target_sched_first_cycle_multipass_dfa_lookahead
...
#define TARGET_SCHED_DEPENDENCIES_EVALUATION_HOOK k1_dependencies_evaluation_hook
