            ...
            for (i = 0; i < nexps; i++)
                XVECEXP (ret, 0, i) =
                    gen_rtx_EXPR_LIST (VOIDmode,
                                       gen_rtx_REG(DImode, K1C_ARGUMENT_POINTER_REGNO+i),
...
k1_expand_prologue(void)
{
  ...
  rtx insn;
  rtx (*gen_set_gotp) (rtx target) = TARGET_64 ? gen_set_gotp_di : gen_set_gotp_si;
...
k1_expand_mov_constant (rtx operands[]) {
  ...
  if (GET_CODE (src) == SYMBOL_REF
	...
	case SYMBOL_GOTOFF:
	  ...
	  emit_move_insn(dest, gen_rtx_PLUS(Pmode,
					    dest, new_rtx));
