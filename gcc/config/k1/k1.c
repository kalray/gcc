...
k1_class_max_nregs(reg_class_t regclass ATTRIBUTE_UNUSED, enum machine_mode mode)
{
...
void
k1_init_cumulative_args (CUMULATIVE_ARGS *cum, const_tree fntype ATTRIBUTE_UNUSED, rtx libname ATTRIBUTE_UNUSED,
...
k1_expand_compare_and_swap (rtx operands[])
{
  ...
  emit_insn (mode == SImode ? gen_acswapw (tmp, mem)
	     : gen_acswapd (tmp, mem));
  ...
  emit_cmp_and_jump_insns (currval, gen_lowpart (mode, high),
			   EQ, NULL_RTX, mode, true, cas_retry);
...
void
k1_expand_atomic_op (enum rtx_code code, rtx target, bool after,
                     rtx mem, rtx val, rtx model)
{
  ...
  switch (code)
    ...
    case MINUS:
    case AND:
      op_res = expand_simple_binop (mode, code, curr_mem_val, val,
                                    new_mem_val, 1, OPTAB_LIB_WIDEN);
      break;
    case MULT: /* NAND */
      {
        rtx x = expand_simple_binop (mode, AND, curr_mem_val, val,
  ...
  emit_cmp_and_jump_insns (gen_lowpart (mode, new_mem_val),
                           const1_rtx, NE, NULL_RTX, mode, true, csloop);
...
#define COMPLEX128 complex_double_type_node
  ...
  tree STRING = build_pointer_type (build_qualified_type (char_type_node, TYPE_QUAL_CONST));
...
#define ADD_K1_BUILTIN(UC_NAME, LC_NAME, ...)                           \
    ...
    ADD_K1_BUILTIN (FADDCDC, "faddcdc",     COMPLEX128, COMPLEX128, COMPLEX128, STRING);
    ...
    ADD_K1_BUILTIN (FSBFCDC, "fsbfcdc",     COMPLEX128, COMPLEX128, COMPLEX128, STRING);
    ...
    ADD_K1_BUILTIN (FMULCDC, "fmulcdc",     COMPLEX128, COMPLEX128, COMPLEX128, STRING);
    ...
    ADD_K1_BUILTIN (FFMAWC,  "ffmawc",      COMPLEX64, COMPLEX64, COMPLEX64, COMPLEX64, STRING);
    ADD_K1_BUILTIN (FFMADC,  "ffmadc",      COMPLEX128, COMPLEX128, COMPLEX128, COMPLEX128, STRING);
    ...
    ADD_K1_BUILTIN (FFMSWC,  "ffmswc",      COMPLEX64, COMPLEX64, COMPLEX64, COMPLEX64, STRING);
    ADD_K1_BUILTIN (FFMSDC,  "ffmsdc",      COMPLEX128, COMPLEX128, COMPLEX128, COMPLEX128, STRING);
...
verify_const_bool_arg (rtx arg)
{
    if (   GET_CODE (arg) != CONST_INT
        || GET_MODE (arg) != VOIDmode) return false;
...
verify_const_field_arg (rtx arg, int bits)
{
    ...
    if (tmp == (stmp<<shift)>>shift) return true;
    if (tmp == (utmp<<shift)>>shift) return true;
    return false;
}
...
k1_expand_builtin_set (rtx target ATTRIBUTE_UNUSED, tree args, bool ps)
{
    ...
    if (!verify_const_int_arg (arg1, 9, false)) {
        error ("__builtin_k1_set expects a 9-bit unsigned immediate first argument.");
        return NULL_RTX;
    }
...
k1_expand_builtin_wfxl (rtx target ATTRIBUTE_UNUSED, tree args)
{
    ...
    rtx arg2 = expand_normal (CALL_EXPR_ARG (args, 1));
    if (!verify_const_int_arg (arg1, 9, false)) {
        error ("__builtin_k1_wfxl expects a 9-bit unsigned immediate first argument.");
        return NULL_RTX;
    }
    int regno = INTVAL (arg1) + K1C_SRF_FIRST_REGNO;
...
k1_expand_builtin_wfxm (rtx target ATTRIBUTE_UNUSED, tree args)
{
    ...
    rtx arg2 = expand_normal (CALL_EXPR_ARG (args, 1));
    if (!verify_const_int_arg (arg1, 9, false)) {
        error ("__builtin_k1_wfxm expects a 9-bit unsigned immediate first argument.");
        return NULL_RTX;
    }
    int regno = INTVAL (arg1) + K1C_SRF_FIRST_REGNO;
    rtx arg = gen_rtx_REG (DImode, regno);
...
static rtx \
k1_expand_builtin_##name (rtx target, tree args) \
{ \
    rtx arg1 = expand_normal (CALL_EXPR_ARG (args, 0)); \
...
K1_EXPAND_BUILTIN_4_MODIFIERS(ffmawd, SFmode, DFmode)
...
K1_EXPAND_BUILTIN_4_MODIFIERS(ffmswd, SFmode, DFmode)
...
k1_expand_builtin_float (rtx target, tree args, bool usigned)
{
    ...
    if (!verify_const_int_arg (arg1, 2, false)) {
        error ("__builtin_k1_float%s expects a 2-bit unsigned immediate first argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }

    if (!verify_const_int_arg (arg3, 6, false)) {
        error ("__builtin_k1_float%s expects a 6-bit unsigned immediate third argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }
...
k1_expand_builtin_floatd (rtx target, tree args, bool usigned)
{
    ...
    if (!verify_const_int_arg (arg1, 2, false)) {
        error ("__builtin_k1_float%sd expects a 2-bit unsigned immediate first argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }

    if (!verify_const_int_arg (arg3, 6, false)) {
        error ("__builtin_k1_float%sd expects a 6-bit unsigned immediate third argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }
...
k1_expand_builtin_fixed (rtx target, tree args, bool usigned)
{
    ...
    if (!verify_const_int_arg (arg1, 2, false)) {
        error ("__builtin_k1_fixed%s expects a 2-bit unsigned immediate first argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }

    if (!verify_const_int_arg (arg3, 6, false)) {
        error ("__builtin_k1_fixed%s expects a 6-bit unsigned immediate third argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }
...
k1_expand_builtin_fixedd (rtx target, tree args, bool usigned)
{
    ...
    if (!verify_const_int_arg (arg1, 2, false)) {
        error ("__builtin_k1_fixed%sd expects a 2-bit unsigned immediate first argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }

    if (!verify_const_int_arg (arg3, 6, false)) {
        error ("__builtin_k1_fixed%sd expects a 6-bit unsigned immediate third argument.",
               usigned ? "u" : "");
        return NULL_RTX;
    }
...
k1_expand_builtin_extfz (rtx target, tree args)
{
    ...
    if (!verify_const_int_arg (arg2, 6, false)) {
        error ("__builtin_k1_extfz expects a 6-bit unsigned immediate second argument.");
        return NULL_RTX;
    }

    if (!verify_const_int_arg (arg3, 6, false)) {
        error ("__builtin_k1_extfz expects a 6-bit unsigned immediate third argument.");
        return NULL_RTX;
    }
                          ...
                          int ignore ATTRIBUTE_UNUSED)
{
    ...
    case K1_BUILTIN_ACSWAPW: return k1_expand_builtin_acswap (target, exp, SImode);
    case K1_BUILTIN_ACSWAPD: return k1_expand_builtin_acswap (target, exp, DImode);
    case K1_BUILTIN_AFADDD: return k1_expand_builtin_afadd (target, exp, DImode);
    case K1_BUILTIN_AFADDW: return k1_expand_builtin_afadd (target, exp, SImode);
...
k1_const_vector_value (rtx x, int index) 
{
  ...
  if (GET_CODE (x) == CONST_VECTOR) {
    ...
    if (inner_mode == HImode) {
      ...
      value = (val_0&0xFFFF) | (val_1&0xFFFF)<<16 | (val_2&0xFFFF)<<32 | (val_3&0xFFFF)<<48;
    } else
    ...
    if (inner_mode == SFmode) {
      ...
      REAL_VALUE_TO_TARGET_SINGLE (*CONST_DOUBLE_REAL_VALUE (elt_0), val[0]);
      REAL_VALUE_TO_TARGET_SINGLE (*CONST_DOUBLE_REAL_VALUE (elt_1), val[1]);
      value = ((HOST_WIDE_INT)val[0]&0xFFFFFFFF) | ((HOST_WIDE_INT)val[1]&0xFFFFFFFF)<<32;
    } else
...
k1_is_uncached_mem_op_p (rtx op)
{
    ...
    gcc_assert (   !MEM_P (op)
                || (MEM_ADDR_SPACE (op) < K1_ADDR_SPACE_CONVERT));
    if (MEM_P (op)) {
...
static void
free_bundle_state (struct bundle_state *state) ATTRIBUTE_UNUSED;
...
k1_gen_bundles (void)
{
  ...
  FOR_EACH_BB_FN (bb, cfun)
					      ...
					      insns */
	{
      ...
      prev = NULL;
      for (rtx_insn *insn = next_insn_to_bundle (BB_HEAD (bb), end);
	   insn;
	   prev = insn, insn = next)
	{
...
static void
k1_dump_bundles(void) ATTRIBUTE_UNUSED;
...
k1_addr_space_convert (rtx op, tree from_type, tree to_type ATTRIBUTE_UNUSED)
{
  if (K1_WARN_ADDRESS_SPACE_CONVERSION
      ...
      && TYPE_ADDR_SPACE (TREE_TYPE (to_type  )) != K1_ADDR_SPACE_CONVERT)
    {
      ...
      warning (0, TYPE_ADDR_SPACE (TREE_TYPE (from_type)) > ADDR_SPACE_GENERIC
