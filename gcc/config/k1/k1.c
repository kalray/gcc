...
{
  if (flag_pic) {
    rtx (*gen_addpcrel) (rtx target, rtx label) = !TARGET_32 ? gen_add_pcrel_di : gen_add_pcrel_si;
...
k1_get_callersaved_nonfixed_reg(machine_mode mode)
{
  ...
  for (regno = 16; regno < FIRST_PSEUDO_REGISTER; regno++)
    {
      if (call_really_used_regs[regno]
	  && ! fixed_regs[regno])
	      ...
	      else
		{
		  ...
		  add_reg_note (insn, REG_CFA_OFFSET,
		  		gen_rtx_SET (spill_mode, mem,
					     (regno == K1C_RETURN_POINTER_REGNO)? orig_save_reg : saved_reg));
		}
	    }
	}
    }
  ...
  if (frame_pointer_needed)
    {
      ...
      insn = emit_insn(gen_add3_insn (hard_frame_pointer_rtx, stack_pointer_rtx,
      ...
      add_reg_note (insn, REG_CFA_DEF_CFA,
		    gen_rtx_PLUS (Pmode,
				  ...
				  GEN_INT (size - frame->hard_frame_pointer_offset)));
    }
}
...
{
    rtx reg;
    rtx (*gen_add) (rtx target, rtx op1, rtx op2) = !TARGET_32 ? gen_adddi3 : gen_addsi3;
...
k1_legitimate_pic_symbolic_ref_p (rtx op)
{
  ...
  return (unspec == UNSPEC_GOT
	  || unspec == UNSPEC_GOTOFF
	  || unspec == UNSPEC_TLS
	  || unspec == UNSPEC_PCREL);
}
...
k1_expand_mov_constant (rtx operands[]) {
  ...
  if (GET_CODE (src) == SYMBOL_REF
      ...
      || GET_CODE (src) == CONST)
    {
      ...
      rtx pic_reg;
      rtx (*gen_set_gotp) (rtx target) = !TARGET_32 ? gen_set_gotp_di : gen_set_gotp_si;
      rtx (*gen_addpcrel) (rtx target, rtx label) = !TARGET_32 ? gen_add_pcrel_di : gen_add_pcrel_si;
...
k1_scan_insn_registers_1 (rtx *x, void *data)
{
	...
	if (GET_MODE_SIZE (GET_MODE (*x)) > UNITS_PER_WORD)
	  {
	    unsigned word;
	    for (word = 1; word < GET_MODE_SIZE (GET_MODE (*x)) / UNITS_PER_WORD; word++)
	      {
	...
	if (GET_MODE_SIZE (GET_MODE (*x)) > UNITS_PER_WORD)
	  {
	    ...
	    for (word = 1; word < GET_MODE_SIZE (GET_MODE (*x)) / UNITS_PER_WORD; word++)
	      {
		SET_HARD_REG_BIT (regs->set_dest ? regs->defs : regs->uses, REGNO (*x) + word);
	      }
	  }
    }
...
k1_gen_bundles (void)
{
  ...
  FOR_EACH_BB_FN (bb, cfun)
					      ...
					      insns */
	{
      ...
      prev = NULL_RTX;
      for (insn = next_insn_to_bundle (BB_HEAD (bb), end);
	   insn;
	   prev = insn, insn = next)
	{
	  ...
	  const int insn_raw = hard_reg_set_intersect_p (cur_insn_regs.uses, cur_bstate->reg_defs);
	  const int insn_waw = hard_reg_set_intersect_p (cur_insn_regs.defs, cur_bstate->reg_defs);
...
k1_swap_fp_sp_in_note (rtx note, rtx old_base)
{
  ...
  if (frame->hard_frame_pointer_offset == 0)
    ...
    {
      HOST_WIDE_INT new_offset = (new_base_reg == hard_frame_pointer_rtx) ? -frame->hard_frame_pointer_offset : frame->hard_frame_pointer_offset;
      if (GET_CODE (XEXP (mem_dest, 0)) == PLUS)
	{
      ...
      XEXP (mem_dest, 0) = gen_rtx_PLUS(Pmode,
					new_base_reg,
	...
	if (RTX_FRAME_RELATED_P (binsn))
	  {
	    ...
	    if (! handled )
	      {
		/* This *must* be some mem write emitted by builtin_save_regs, or a bug */
      ...
      /* Iterate from start and fix possible MEM using the incorrect base register */
