...
initial stack pointer        |                               |
			     ...
			     |            padding            |
                             ...
                             +-------------------------------+
                             |                               | ^
                             | scratch area                  | | STACK_POINTER_OFFSET (K1C_SCRATCH_AREA_SIZE)
                             |                               | v
                             +-------------------------------+ <---- stack pointer
...
k1_compute_frame_info (void)
{
 ...
 /* Next is the callee-allocated area for pretend stack arguments.  */
  ...
  frame->initial_sp_offset = (offset <= K1C_SCRATCH_AREA_SIZE) ?
    0 : offset - K1C_SCRATCH_AREA_SIZE;
...
static unsigned int disabled_isa_mask_k1tiny =
  K1_ISA_ALU_LITE | K1_ISA_ALU_FULL | K1_ISA_ALU_FULL_ODD
  | K1_ISA_MAU_FPU;
...
static rtx
k1_get_arg_info(struct k1_arg_info *info,
		...
		bool named ATTRIBUTE_UNUSED)
{
  ...
  HOST_WIDE_INT n_bytes = type ? int_size_in_bytes(type) : GET_MODE_SIZE(mode);
...
/* Implements TARGET_FUNCTION_ARG_BOUNDARY.
   Align arguments on a 64bits boundary for small arguments, 128 for bigger ones. */

static unsigned int
k1_function_arg_boundary (enum machine_mode mode, const_tree type)
{
  HOST_WIDE_INT sz = (mode == BLKmode ? int_size_in_bytes (type) : GET_MODE_SIZE (mode));
...
k1_target_return_in_memory (const_tree type, const_tree fntype ATTRIBUTE_UNUSED)
{
  ...
  return TYPE_MODE (type) == BLKmode
    && (sz > (K1C_ARG_REG_SLOTS * UNITS_PER_WORD)
	|| sz < 0);
}
...
static int k1_target_sched_adjust_cost (rtx insn, rtx link, rtx dep_insn, int cost)
{
