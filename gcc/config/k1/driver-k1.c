...
k1_tooldir (int argc, const char **argv)
{
    ...
    res = concat (gcc_exec_prefix, dir_separator,
                  DEFAULT_TARGET_MACHINE, dir_separator,
                  DEFAULT_TARGET_VERSION, dir_separator,
    ...
    if (!access (res, R_OK|X_OK)) {
        if (len && res[len-1] == ' ')  res[len-1] = DIR_SEPARATOR;
        return res;
    }

    res2 = concat (STANDARD_EXEC_PREFIX, dir_separator,
                   DEFAULT_TARGET_MACHINE, dir_separator,
                   DEFAULT_TARGET_VERSION, dir_separator,
                   TOOLDIR_BASE_PREFIX, NULL);
    ...
    if (!access (res2, R_OK|X_OK)) {
        if (len && res2[len-1] == ' ')  res2[len-1] = DIR_SEPARATOR;
        return res2;
    }

    len = strlen(res);
    if (res[len-1] == ' ')  res[len-1] = DIR_SEPARATOR;
    return res;
}
...
k1_board_to_startfile_prefix (int argc, const char **argv)
{
	...
	/* Seamlessly change the bsp to csp_generic if developer, emb01, ab04, konic80, or tc */
	if (strcmp(res, "developer/") == 0 || 
			strcmp(res, "emb01/") == 0 ||
			strcmp(res, "ab04/") == 0 ||
			strcmp(res, "tc2/") == 0 ||
			strcmp(res, "tc3/") == 0 ||
			strcmp(res, "konic80/") == 0) {
		res = "csp_generic/";
...
k1_cluster_board_to_bsp (int argc, const char **argv)
{
	...
	/* Seamlessly change the bsp to csp_generic if developer, emb01, ab04, konic80, or tc */
	if (!strcmp(res, "developer") || !strcmp(res, "emb01") || !strcmp(res, "ab04") || !strcmp(res, "tc2") || !strcmp(res, "tc3") || !strcmp(res, "konic80")) {
		fprintf(stderr, "board_to_bsp: Changing board type from %s to csp_generic\n", res);
		res = "csp_generic";
	}
