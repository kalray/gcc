...
#define LINK_GCC_C_SEQUENCE_SPEC "%{!nostdlib:-( %G %L -lgloss -lnodeos %{pthread*:} %{mhypervisor: -whole-archive -lmOS -no-whole-archive -lvbsp}  -lrt -lboard -lmppa_fdt -lcore %{!T*: -dTnodeos.ld }  %G %L -) }"
...
#define K1_SELECT_DP_CORE "%{march=k1b:-mcore=k1bdp;:-mcore=" DEFAULT_DP_CORE "}"
#define K1_DEFAULT_ARCH "k1b"

#define K1_OS_SELF_SPECS                                                         \
  "%{!march*:%{mcore=k1b*:-march=k1b;:-march=" K1_DEFAULT_ARCH "}} ",						\
  "%{!mcore*:%{mcluster=io*:%eOnly compute cluster (node) is supported;:" K1_SELECT_DP_CORE "}} ", \
  "%{!mcluster*:%{mcore=k1bio:%eOnly compute cluster (node) is supported;mcore=k1bdp:-mcluster=node;:-mcluster=node}} ", \
        "%{!mgprel: %{!mno-gprel: -mno-gprel}}", \
        "%{!mno-hypervisor: -mhypervisor}",				\
