...
#define DRIVER_SELF_SPECS_COMMON                                      \
	...
	"%{mcore=k1bio: -fstrict-align} ",			      \
        ...
        "%{fpic: %<mgprel } %{fPIC: %<mgprel }",                      \
        "%{show: -v} %<show"
/*
        "%{!mno-gprel: -mgprel}"                        \
*/

#define CPP_SPEC_COMMON "%{mcluster=*:%:cluster_to_define(%{mcluster=*:%*})} " \
...
#define EXTRA_SPEC_FUNCTIONS                                     \
  ...
  { "concat",                     k1_concat},                    \
  { "cluster_to_define",          k1_cluster_to_define},         \
...
#define HARD_REGNO_NREGS(REGNO, MODE)					\
  ((REGNO_REG_CLASS(REGNO) == SRF64_REGS && MODE == DImode) ? 1 :	\
   ((GET_MODE_SIZE (MODE) + UNITS_PER_WORD - 1)				\
    / (UNITS_PER_WORD)))						\

/* If defined, a C expression that returns nonzero for a class for
   which a change from mode from to mode to is invalid. */
#define CANNOT_CHANGE_MODE_CLASS(from, to, class) k1_cannot_change_mode_class(from,to,class)

/* A C expression that is nonzero if it is permissible to store a
   value of mode mode in hard register number regno (or in several
   registers starting with that one). */
#define HARD_REGNO_MODE_OK(REGNO, MODE) ((GET_MODE_SIZE (MODE) <= 4 &&  REGNO_REG_CLASS(REGNO) != SRF64_REGS) \
...
#define ADDITIONAL_REGISTER_NAMES { \
    { "p0" ,    0  }, { "p2",     2  }, { "p4",     4  }, { "p6",     6  }, \
    { "p8" ,    8  }, { "p10",    10 }, { "p12",    12 }, { "p14",    14 }, \
    { "p16",    16 }, { "p18",    18 }, { "p20",    20 }, { "p22",    22 }, \
    { "p24",    24 }, { "p26",    26 }, { "p28",    28 }, { "p30",    30 }, \
    { "p32",    32 }, { "p34",    34 }, { "p36",    36 }, { "p38",    38 }, \
    { "p40",    40 }, { "p42",    42 }, { "p44",    44 }, { "p46",    46 }, \
    { "p48",    48 }, { "p50",    50 }, { "p52",    52 }, { "p54",    54 }, \
    { "p56",    56 }, { "p58",    58 }, { "p60",    60 }, { "p62",    62 }, \
    { "r0r1",   0  }, { "r2r3",   2  }, { "r4r5",   4  }, { "r6r7",   6  }, \
    ...
    { "s0",     64 }, { "s1",     65 }, { "s2",     66 }, { "s3",     67 }, \
    { "s4",     68 }, { "s5",     69 }, { "s6",     70 }, { "s7",     71 }, \
    { "s8",     72 }, { "s9",     73 }, { "s10",    74 }, { "s11",    75 }, \
    { "s12",    76 }, { "s13",    77 }, { "s14",    78 }, { "s15",    79 }, \
    { "s16",    80 }, { "s17",    81 }, { "s18",    82 }, { "s19",    83 }, \
    { "s20",    84 }, { "s21",    85 }, { "s22",    86 }, { "s23",    87 }, \
    { "s24",    88 }, { "s25",    89 }, { "s26",    90 }, { "s27",    91 }, \
    { "s28",    92 }, { "s29",    93 }, { "s30",    94 }, { "s31",    95 }, \
    { "s32",    96 }, { "s33",    97 }, { "s34",    98 }, { "s35",    99 }, \
    { "s36",   100 }, { "s37",   101 }, { "s38",   102 }, { "s39",   103 }, \
    { "s40",   104 }, { "s41",   105 }, { "s42",   106 }, { "s43",   107 }, \
    { "s44",   108 }, { "s45",   109 }, { "s46",   110 }, { "s47",   111 }, \
    { "s48",   112 }, { "s49",   113 }, { "s50",   114 }, { "s51",   115 }, \
    { "s52",   116 }, { "s53",   117 }, { "s54",   118 }, { "s55",   119 }, \
    { "s56",   120 }, { "s57",   121 }, { "s58",   122 }, { "s59",   123 }, \
    { "s60",   124 }, { "s61",   125 }, { "s62",   126 }, { "s63",   127 }, \
}
...
#define CLZ_DEFINED_VALUE_AT_ZERO(mode, value)   \
    (mode == SImode ? (value = 32, 1)	         \
     : ( mode == DImode ? (value = 64, 1) : 0 ))
#define CTZ_DEFINED_VALUE_AT_ZERO(mode, value) \
    CLZ_DEFINED_VALUE_AT_ZERO(mode, value)

#define TARGET_EXTRA_PRE_INCLUDES k1_target_extra_pre_includes

#define FINAL_PRESCAN_INSN(insn,ops,nops) k1_final_prescan_insn(insn,ops,nops)

extern void k1_target_extra_pre_includes (const char *sysroot, const char *iprefix, int stdinc);
...
#define k1_strict_to_nonstrict_comparison_operator(code)  \
    ...
    enum rtx_code comp;					  \
							  \
    switch (code) {					  \
    case GT: comp = GE; break;				  \
    case GE: comp = GT; break;				  \
    case GTU: comp = GEU; break;			  \
    case GEU: comp = GTU; break;			  \
    case LT: comp = LE; break;				  \
    case LE: comp = LT; break;				  \
    case LTU: comp = LEU; break;			  \
    case LEU: comp = LTU; break;			  \
