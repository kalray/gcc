...
kvx_expand_conditional_move (rtx target, rtx select1, rtx select2, rtx comp)
{
  ...
  if (vector_modes_p)
    {
      ...
      vector_any_and_mone &= - (pred_mode == mode);
    }
  ...
  if (vector_any_and_zero || vector_any_and_mone)
	...
	{
	  select1 = force_reg (mode, select1);
	  rtx andp = gen_rtx_UNSPEC (mode, gen_rtvec (2, select1, pred), UNSPEC_ANDD);
	  set = emit_insn (gen_rtx_SET (dst, andp));
	}
      if (vector_any_and_zero < 0 && select2 != constm1_mode_rtx)
	{
	  select2 = force_reg (mode, select2);
	  rtx andp = gen_rtx_UNSPEC (mode, gen_rtvec (2, select2, pred), UNSPEC_ANDD);
	  set = emit_insn (gen_rtx_SET (dst, andp));
	}
      if (vector_any_and_mone > 0 && select1 != const0_mode_rtx)
	{
	  select1 = force_reg (mode, select1);
	  rtx iorp = gen_rtx_UNSPEC (mode, gen_rtvec (2, select1, pred), UNSPEC_IORD);
	  set = emit_insn (gen_rtx_SET (dst, iorp));
	}
      if (vector_any_and_mone < 0 && select2 != const0_mode_rtx)
	{
	  select2 = force_reg (mode, select2);
	  rtx iorp = gen_rtx_UNSPEC (mode, gen_rtvec (2, select2, pred), UNSPEC_IORD);
	  set = emit_insn (gen_rtx_SET (dst, iorp));
	}
      if (!set)
	{
    ...
    // General case, do a conditional move with test `PRED NE CONST0`.
...
kvx_expand_masked_move (rtx target, rtx select1, rtx select2, rtx pred)
{
  ...
  if (vector_any_and_zero || vector_any_and_mone)
    ...
    // General case, do a conditional move with test `PRED COMP_CODE CONST0`.
    // The combiner should then fold the comparaison into the conditional move test.
    {
...
static struct kvx_ifcvt
{
  ...
  struct { rtx_insn *insn; rtx dest_reg; basic_block block; }
    spec_insns[2*MAX_CONDITIONAL_EXECUTE];
  int spec_insns_count;
  struct { rtx_insn *insn; rtx parallel; basic_block block; }
...
{
  memset (kvx_ifcvt.spec_insns, 0, sizeof (kvx_ifcvt.spec_insns));
  kvx_ifcvt.spec_insns_count = 0;
...
static rtx
kvx_ifcvt_get_reg_test (basic_block block)
{
...
/* Check that an insn with this PATTERN is recognized.  If SPLIT is FALSE,
   reject patterns whose ASM template is '#' (pattern must be split).  */
static bool
...
static bool
kvx_ifcvt_ce2_cond_move_ce3 (rtx_insn *insn, rtx reg_test, basic_block block)
{
  ...
  if (register_operand (set_dest, VOIDmode)
    ...
    {
      rtx new_pattern = gen_rtx_COND_EXEC (VOIDmode, reg_test, pattern);
      bool recog =  kvx_ifcvt_ce2_recog_pattern (new_pattern, true);
      return recog;
    }
  ...
  if (!memsimple)
    {
      ...
      XVECEXP (parallel, 0, 1) = gen_rtx_USE (VOIDmode, reg);
      ...
      kvx_ifcvt.prep_insns[kvx_ifcvt.prep_insns_count].block = block;
...
static bool
kvx_ifcvt_ce2_cond_arith_ce3 (rtx_insn *insn, rtx reg_test, basic_block block)
{
  rtx pattern = PATTERN (insn);
  gcc_checking_assert (GET_CODE (pattern) == SET);
  ...
  rtx new_pattern = gen_rtx_COND_EXEC (VOIDmode, reg_test, parallel);
  bool recog = kvx_ifcvt_ce2_recog_pattern (new_pattern, true);
  return recog;
}

static bool
kvx_ifcvt_ce2_spec_arith_ce3 (rtx_insn *insn, rtx reg_test, basic_block block)
{
  rtx pattern = PATTERN (insn);
  gcc_checking_assert (GET_CODE (pattern) == SET);
  ...
  if (REG_P (set_dest))
    dest_reg = set_dest;
  else if (SUBREG_P (set_dest) && REG_P (SUBREG_REG (set_dest)))
    dest_reg = SUBREG_REG (set_dest);
  else
    return false;

  regset live_out = df_get_live_out (block);
  if (!REGNO_REG_SET_P (live_out, REGNO (dest_reg)))
    {
      gcc_assert (kvx_ifcvt.spec_insns_count < 2*MAX_CONDITIONAL_EXECUTE);
      kvx_ifcvt.spec_insns[kvx_ifcvt.spec_insns_count].dest_reg = dest_reg;
      kvx_ifcvt.spec_insns[kvx_ifcvt.spec_insns_count].block = block;
      kvx_ifcvt.spec_insns[kvx_ifcvt.spec_insns_count].insn = insn;
      kvx_ifcvt.spec_insns_count++;
      return true;
    }
...
/* Check if BLOCK only contains valid candidates for if-conversion in CE3.
   Valid candidates are object move to register or load and stores, which have
   a recognized COND_EXEC variant.  The side-effect free instructions can be
   speculatively executed provided the live range of their definition is
   suitably extended.  */
static bool
  ...
  FOR_BB_INSNS (block, insn)
      ...
      if (NONJUMP_INSN_P (insn))
	{
	  ...
	  if (count++ >= MAX_CONDITIONAL_EXECUTE)
	    {
if (kvx_ifcvt_dump) { fprintf(stderr, "NOT CANDIDATE (MAX_CONDITIONAL):\n"); }
	      return false;
	    }
	  if (side_effects_p (pattern))
	    {
if (kvx_ifcvt_dump) { fprintf(stderr, "NOT CANDIDATE (SIDE_EFFECTS):\n"); debug_rtx (insn); }
	      return false;
	    }
	  if (GET_CODE (pattern) != SET)
	    {
if (kvx_ifcvt_dump) { fprintf(stderr, "NOT CANDIDATE (NOT SET):\n"); debug_rtx (insn); }
	      return false;
	    }
	  ...
	  if (kvx_ifcvt_ce2_cond_move_ce3 (insn, reg_test, block))
	    {
if (kvx_ifcvt_dump) { fprintf(stderr, "COND_MOVE (# %d):\n", INSN_UID (insn)); }
	      continue;
	    }
	  ...
	  if (contains_mem_rtx_p (pattern))
		...
		{
if (kvx_ifcvt_dump) { fprintf(stderr, "COND_MEM (# %d):\n", INSN_UID (insn)); }
		  continue;
		}
	    }
		...
		{
if (kvx_ifcvt_dump) { fprintf(stderr, "SPEC_ARTITH (# %d):\n", INSN_UID (insn)); }
		  continue;
		}

	      if (kvx_ifcvt_ce2_cond_arith_ce3 (insn, reg_test, block))
		{
if (kvx_ifcvt_dump) { fprintf(stderr, "COND_ARTITH (# %d):\n", INSN_UID (insn)); }
		  continue;
		}
	    }

if (kvx_ifcvt_dump) { fprintf(stderr, "NOT CANDIDATE 2:\n"); debug_rtx (insn); }
	    return false;
	}
      else if (INSN_P (insn))
      //else if (NONDEBUG_INSN_P (insn))
	{
if (kvx_ifcvt_dump && !CALL_P(insn)) { fprintf(stderr, "NOT CANDIDATE 3:\n"); debug_rtx (insn); }
	  return false;
	}
    }
			...
			int *_cond_exec_changed_p)
{
  ...
  // Here in CE2, construct kvx_ifcvt not already done.
  if (!kvx_ifcvt.block_count)
    kvx_ifcvt_ctor ();
  else
    kvx_ifcvt_reset ();
...
if (kvx_ifcvt_dump)
  {
    fprintf (stderr, "KVX_IFCVT_COUNT=%d\n", kvx_ifcvt_count + 1);
    fprintf (stderr, "    (%s)\n", current_function_name ());
    fprintf (stderr, "\ttest-block %d\n", ce_info->test_bb->index);
    fprintf (stderr, "\tthen-block %d\n", then_bb ? then_bb->index : -1);
    fprintf (stderr, "\telse-block %d\n", else_bb ? else_bb->index : -1);
    fprintf (stderr, "\tjoin-block %d\n", join_bb ? join_bb->index : -1);
    if (!candidate)
      fprintf (stderr, "\tNO IFCVT CE3\n");
    else
      fprintf (stderr, "\tIFCVT CE3 CANDIDATE\n");
  }
  ...
  if (then_bb)
	...
	last_insn = PREV_INSN (last_insn);
      ...
      gen_rtx_USE (VOIDmode, tested_reg);
  ...
  // Find the last insertion point in test_bb.
  rtx_insn *split_insn = BB_END (test_bb);
  if (JUMP_P (split_insn))
    split_insn = PREV_INSN (split_insn);

  // Find the first insertion point in join_bb.
  rtx_insn *join_insn = BB_HEAD (join_bb);
  while (join_insn && LABEL_P (join_insn))
    join_insn = NEXT_INSN (join_insn);
  gcc_assert (NOTE_INSN_BASIC_BLOCK_P (join_insn));
  ...
  for (int index = 0; index < kvx_ifcvt.prep_insns_count; index++)
    {
      ...
      basic_block block = kvx_ifcvt.prep_insns[index].block;
      ...
      df_insn_rescan (insn);
      changed = true;
if (kvx_ifcvt_dump) { fprintf(stderr, "PREPARE COND_EXEC(# %d)\n", INSN_UID (insn)); }
    }

  // Process the speculated insns.
  for (int index = 0; index < kvx_ifcvt.spec_insns_count; index++)
    {
      rtx_insn *insn = kvx_ifcvt.spec_insns[index].insn;
      rtx dest_reg = kvx_ifcvt.spec_insns[index].dest_reg;
      // Use the REG_NONNEG note to flag insn as CE3 speculative.
      add_reg_note (insn, REG_NONNEG, NULL_RTX);

      // Insert USE of dest_reg in join_bb.
      join_insn = emit_insn_after (gen_rtx_USE (VOIDmode, dest_reg), join_insn);
      df_set_bb_dirty (join_bb);
      changed = true;
if (kvx_ifcvt_dump) { fprintf(stderr, "PREPARE SPECULATED(# %d)\n", INSN_UID (insn)); }
    }

  // Add DEFs in test_bb and USEs in join_bb of scratch registers.
  for (int index = 0; index < kvx_ifcvt.prep_insns_count; index++)
    {
      rtx_insn *insn = kvx_ifcvt.prep_insns[index].insn;
      rtx parallel = kvx_ifcvt.prep_insns[index].parallel;
      rtx scratch_reg = XEXP (XVECEXP (parallel, 0, 1), 0);
      gcc_checking_assert (REG_P (scratch_reg));
      // Insert DEF of scratch_reg with unique value in test_bb.
      rtvec vec = gen_rtvec (1, GEN_INT (REGNO (scratch_reg)));
      rtx def = gen_rtx_UNSPEC (GET_MODE (scratch_reg), vec, UNSPEC_DEF);
      split_insn = emit_insn_after (gen_rtx_SET (scratch_reg, def), split_insn);
      // Insert USE of scratch_reg in join_bb.
      join_insn = emit_insn_after (gen_rtx_USE (VOIDmode, scratch_reg), join_insn);
    }
  if (kvx_ifcvt.prep_insns_count)
    {
...
kvx_option_override (void)
{
  ...
  const char *KVX_IFCVT_DUMP = getenv("KVX_IFCVT_DUMP");
  if (KVX_IFCVT_DUMP)
    kvx_ifcvt_dump = atoi (KVX_IFCVT_DUMP);
